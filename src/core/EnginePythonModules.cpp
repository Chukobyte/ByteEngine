...
PyObject* EnginePythonModules::game_create_label_entity(PyObject* self, PyObject* args, PyObject* kwargs) {
    ...
    if(PyArg_ParseTupleAndKeywords(args, kwargs, "ss(ii)si|bi(iii)", gameCreateLabelEntitykwlist,
        ...
        */
        float red = static_cast<float>(pyRed * RGB_WHOLE_NUMBER_COLOR_CONVERSION);
        float green = static_cast<float>(pyGreen * RGB_WHOLE_NUMBER_COLOR_CONVERSION);
        float blue = static_cast<float>(pyBlue * RGB_WHOLE_NUMBER_COLOR_CONVERSION);
